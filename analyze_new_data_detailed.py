#!/usr/bin/env python3
"""Detailed analysis of NEW data for nitrates pipeline with sample ID matching focus."""
...
print("=" * 100)
print("NITRATES PIPELINE - DETAILED NEW DATA ANALYSIS")
...
# Analyze nitrate column
print("\n" + "-" * 100)
print("Nitrate Data Analysis:")
print("-" * 100)

# Try to find the nitrate column - handle both formats (newline vs space)
...
if k_col not in df_ref.columns:
    ...
    if k_col_newline in df_ref.columns:
        ...
        # Try to find any column containing nitrate/NO3
        k_cols = [c for c in df_ref.columns if 'NO3' in c or 'Nitrate' in c or 'nitrate' in c]
        if k_cols:
            k_col = k_cols[0]
            print(f"⚠️ Using detected NO3 column: '{k_col}'")
        else:
            print(f"❌ Could not find nitrate column. Available columns: {list(df_ref.columns)}")
            k_col = None
...
if k_col and k_col in df_ref.columns:
    ...
    # Show rows with NO3 values and their IDs
    print("\n  Sample rows with NO3 values (first 10):")
...
if all_files:
    ...
    # Pattern: Example filename format for nitrates data
    # Let's extract different parts
    sample_ids_extracted = []
    sample_numbers = []
    cngs_numbers = []

    # Adjust pattern based on actual filename format (generic for nitrates)
    pattern = r'MPN_\d+_\d+_(CNGS\d+)_\w+_\d+_(S\d+)_'
...
if k_col:
    print(f"  • Samples with NO3 values: {df_ref[k_col].notna().sum()} ({df_ref[k_col].notna().sum()/len(df_ref)*100:.1f}%)")
    print(f"  • Missing NO3 values: {df_ref[k_col].isna().sum()}")
    print(f"  • NO3 concentration range: {k_data.min():.3f} - {k_data.max():.3f}%")
else:
    print(f"  ⚠️ Nitrate column not found in reference data")
...
if k_col and k_col in df_ref.columns:
    ...
    # Main title
    fig.suptitle('NEW Data - Nitrate (NO3) Concentration Analysis\n' +
                 f'{config.reference_data_path.name} ({len(k_data)} samples)',
    ...
    ax1.set_xlabel('Nitrate NO3 (%)', fontsize=12)
    ...
    ax2.set_ylabel('Nitrate NO3 (%)', fontsize=12)
    ...
    ax3.set_xlabel('Nitrate NO3 (%)', fontsize=11)
    ...
    ax5.set_ylabel('Nitrate NO3 (%)', fontsize=11)
