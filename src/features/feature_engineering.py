...
class RawSpectralTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        total_values = df_result.shape[0] * df_result.shape[1]
        nan_count = df_result.isnull().sum().sum()
        nan_percentage = (nan_count / total_values * 100) if total_values > 0 else 0.0

        logger.debug(
            "[RAW SPECTRAL] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
        ...
        if df_result.isnull().any().any():
            ...
            # CRITICAL ERROR: NaN values should not occur in raw spectral extraction
            error_msg = (
                f"[RAW SPECTRAL] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"across {len(nan_cols)} features in {len(elements_with_nan)} elements.\n"
                f"Elements with NaNs: {list(elements_with_nan.keys())}\n"
                f"Sample columns: {nan_cols[:10]}\n"
                f"This indicates a problem with spectral data extraction or region isolation."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)

        logger.info(
            "[RAW SPECTRAL] Extracted %d raw intensity features from %d PeakRegions.",
...
class SpectralFeatureGenerator(BaseEstimator, TransformerMixin):
    ...
    def _get_strategy_regions(self) -> List[PeakRegion]:
        ...
        if self.strategy == "N_only":
            # N regions + essential context elements for meaningful ratios
            n_regions = [self.config.nitrate_region]
            n_regions.extend([r for r in self.config.context_regions if r.element.startswith("N_I")])

            # Add C_I for N_C_ratio calculation (baseline normalization)
            c_region = next((r for r in self.config.context_regions if r.element == "C_I"), None)
            if c_region:
                n_regions.append(c_region)

            # Add critical elements for nutrient balance (K, Ca, P for NPK context)
            critical_elements = ['K_I', 'CA_I_help', 'CA_II_393', 'P_I_secondary']
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # Extract N_I_primary, K_I and C_I areas for ratio calculation
        if "N_I_primary_peak_0" not in base_features_df:
            raise ValueError("N_I_primary_peak_0 not found in base features. Ensure nitrogen (N_I_primary) region is properly extracted for nitrate prediction.")
        if "K_I_peak_0" not in base_features_df:
            raise ValueError("K_I_peak_0 not found in base features. Ensure potassium context features are properly extracted for nitrate prediction.")
        ...
        n_area = base_features_df["N_I_primary_peak_0"].fillna(0.0)
        k_area = base_features_df["K_I_peak_0"].fillna(0.0)
        c_area = base_features_df["C_I_peak_0"].fillna(1e-6)

        # Calculate N/C ratio (primary indicator for nitrate) with clipping to prevent extreme values
        nc_ratio_raw = n_area / c_area.replace(0, 1e-6)
        base_features_df["N_C_ratio"] = np.clip(nc_ratio_raw, -50.0, 50.0)

        # Calculate K/C ratio (nutrient balance context) with clipping to prevent extreme values
        ...
        if len(expected_features) != len(set(expected_features)):
            ...
            if self.strategy == "full_context":
                ...
                logger.error("High NO3 features count: %d", len(self._high_no3_names))
                logger.error(
        ...
        logger.debug(
            "[FEATURE ENGINEERING] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )
        ...
        if final_df.isnull().any().any():
            ...
            # CRITICAL ERROR: NaN values should not occur after feature engineering
            error_msg = (
                f"[FEATURE ENGINEERING] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"across {len(nan_cols)} features.\n"
                f"Sample NaN features: {nan_cols[:20]}\n"
                f"Strategy: {self.strategy}\n"
                f"This indicates a problem with feature extraction, ratio calculation, or feature generation."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)

        logger.info(
            "[FEATURE ENGINEERING] Transformed data into %d features for strategy '%s'.",
    ...
    def _set_feature_names(self, X_sample):
        ...
        sample_base_df["N_C_ratio"] = 0.0  # Placeholder for nitrate/carbon ratio
        sample_base_df["K_C_ratio"] = 0.0  # Placeholder for potassium/carbon ratio
        ...
        if self.strategy == "N_only":
            # Include all N_I features (nitrogen atomic lines) and N_C_ratio
            ...
            # Include all enhanced N features (ratios now have real values from extracted regions)
            n_enhanced = [
                name for name in enhanced_names
                if "N" in name or "nitrate" in name.lower() or "nitrogen" in name.lower()
            ]
            # NEW: Include physics-informed features for N
            n_physics = [name for name in physics_informed_names if name.startswith("N_I")]

            # Include context features (K, C, P for nutrient balance)
            context_features = [name for name in self._all_simple_names if any(name.startswith(elem) for elem in ["K_I", "C_I", "P_I", "CA_"])]

            # Always add N_C_ratio (critical nitrogen indicator, computed separately) and K_C_ratio for context
        ...
        if duplicates:
            ...
            logger.error("  High NO3 names: %s", self._high_no3_names)
