...
class PCAReducer(DimensionReducer):
    ...
    def transform(self, X):
        ...
        total_values = X_transformed.size
        nan_count = np.isnan(X_transformed).sum()
        nan_percentage = (nan_count / total_values * 100) if total_values > 0 else 0.0

        logger.debug(
            "[PCA REDUCER] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after PCA dimensionality reduction
            error_msg = (
                f"[PCA REDUCER] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"in PCA-transformed features.\n"
                f"Components: {self.n_components}\n"
                f"This indicates a problem with PCA transformation or input data."
            )
...
class PLSReducer(DimensionReducer):
    ...
    def transform(self, X):
        ...
        logger.debug(
            "[PLS REDUCER] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after PLS dimensionality reduction
            error_msg = (
                f"[PLS REDUCER] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"in PLS-transformed features.\n"
                f"Components: {self.n_components}\n"
                f"This indicates a problem with PLS transformation or input data."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)
...
class AutoencoderReducer(DimensionReducer):
    ...
    def transform(self, X):
        ...
        X_transformed = encoded.cpu().numpy()
        ...
        logger.debug(
            "[AUTOENCODER REDUCER] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after Autoencoder dimensionality reduction
            error_msg = (
                f"[AUTOENCODER REDUCER] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"in Autoencoder-transformed features.\n"
                f"Latent dimensions: {self.n_components}\n"
                f"This indicates a problem with autoencoder transformation or input data."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)
...
class FeatureClusteringReducer(DimensionReducer):
    ...
    def transform(self, X):
        ...
        # NaN statistics logging
        total_values = X_reduced.size
        nan_count = np.isnan(X_reduced).sum()
        nan_percentage = (nan_count / total_values * 100) if total_values > 0 else 0.0

        logger.debug(
            "[FEATURE CLUSTERING REDUCER] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after Feature Clustering dimensionality reduction
            error_msg = (
                f"[FEATURE CLUSTERING REDUCER] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"in Feature Clustering-transformed features.\n"
                f"Clusters: {self.n_components}\n"
                f"This indicates a problem with feature clustering or input data."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)
