...
patterns specific to different nitrate concentration ranges, replacing
...
class MinimalConcentrationFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # NaN statistics logging
        total_values = X_df.shape[0] * X_df.shape[1]
        nan_count = X_df.isnull().sum().sum()
        nan_percentage = (nan_count / total_values * 100) if total_values > 0 else 0.0

        logger.debug(
            "[MINIMAL CONCENTRATION FEATURES] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
        ...
        if nan_count > 0:
            ...
            # CRITICAL ERROR: NaN values should not occur in minimal concentration features
            error_msg = (
                f"[MINIMAL CONCENTRATION FEATURES] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"across {len(nan_cols)} features.\n"
                f"NaN features: {nan_cols}\n"
                f"This indicates a problem with raw spectral data or intensity proxy calculation."
            )
...
class ConcentrationRangeFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # NaN statistics logging
        total_values = X_enhanced.shape[0] * X_enhanced.shape[1]
        nan_count = X_enhanced.isnull().sum().sum()
        nan_percentage = (nan_count / total_values * 100) if total_values > 0 else 0.0

        logger.debug(
            "[CONCENTRATION RANGE FEATURES] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )
        ...
        if nan_count > 0:
            ...
            # CRITICAL ERROR: NaN values should not occur in concentration range features
            error_msg = (
                f"[CONCENTRATION RANGE FEATURES] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"across {len(nan_cols)} features.\n"
                f"Sample NaN features: {nan_cols[:20]}\n"
                f"This indicates a problem with range indicators, spectral modulation, ratio adjustments, or concentration interactions."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)
    ...
    def _learn_data_characteristics(self, X: pd.DataFrame, y: Optional[pd.Series] = None):
        ...
        if 'K_C_ratio' in X.columns:
            ...
            raise ValueError("K_C_ratio not found in features. Ensure features are properly extracted for nitrate prediction.")
        ...
        # Check if C_I features exist (used for K/C ratio)
        ...
        # Note: K_I already added above, so exclude it from this loop
        for element in ['N_I', 'H_I', 'O_I', 'CA_I', 'S_I', 'P_I']:
            if any(element in col for col in X.columns):
                if element not in elements_to_track:  # Avoid duplicates
    ...
    def _add_range_indicators(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if hasattr(self, 'k_intensity_percentiles_') and self.k_intensity_percentiles_:
            ...
            if k_intensity_cols:
                ...
                raise ValueError("No K_I intensity features found. Ensure features are properly extracted for nitrate prediction.")
        else:
            raise ValueError("No K_I intensity percentiles found. Ensure features are properly extracted for nitrate prediction.")
        ...
        if 'K_C_ratio' in X.columns and hasattr(self, 'kc_ratio_percentiles_') and self.kc_ratio_percentiles_:
            ...
            raise ValueError("K_C_ratio not found or no ratio percentiles available. Ensure features are properly extracted for nitrate prediction.")
    ...
    def _add_spectral_modulation(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if hasattr(self, 'k_intensity_percentiles_') and self.k_intensity_percentiles_:
            ...
            raise ValueError("No K_I intensity percentiles available. Ensure features are properly extracted for nitrate prediction.")
...
def create_enhanced_feature_pipeline_with_concentration(config, strategy: str, exclude_scaler: bool = False, use_parallel: bool = False, n_jobs: int = -1):
    ...
    if config.use_raw_spectral_data:
        ...
        if strategy == "N_only":
            # Focus on N-specific concentration patterns
            concentration_config = {
                'enable_range_indicators': True,
                'enable_spectral_modulation': True,
                'enable_ratio_adjustments': True,  # N_C_ratio and K_C_ratio are included in N_only
