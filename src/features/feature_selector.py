...
high-dimension/low-sample scenarios in the nitrate prediction pipeline.
"""
    ...
    def fit(self, X: Union[np.ndarray, pd.DataFrame], y: Union[np.ndarray, pd.Series]):
        ...
        if isinstance(X, pd.DataFrame):
            if X.isnull().any().any():
                ...
                # CRITICAL ERROR: NaN values should not occur in feature selector input
                error_msg = (
                    f"[FEATURE SELECTION FIT] CRITICAL: Found {nan_count} NaN values in input features.\n"
                    f"NaN columns: {nan_cols[:20]}\n"
                    f"Feature selection cannot operate on NaN values. Fix upstream feature generation."
                )
    ...
    def transform(self, X: Union[np.ndarray, pd.DataFrame]) -> Union[np.ndarray, pd.DataFrame]:
        ...
        if self.selector in ['lasso', 'tree_importance']:
            ...
            if isinstance(X, pd.DataFrame):
                ...
                X_transformed = X[selected_columns]
            else:
            ...
            if isinstance(X, pd.DataFrame) and self.feature_names_:
                selected_names = [name for name, selected in zip(self.feature_names_, self.selected_features_) if selected]
                X_transformed = pd.DataFrame(X_transformed, columns=selected_names, index=X.index)

        # NaN statistics logging
        if isinstance(X_transformed, pd.DataFrame):
            total_values = X_transformed.shape[0] * X_transformed.shape[1]
        ...
        logger.debug(
            "[FEATURE SELECTOR] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after feature selection
            if isinstance(X_transformed, pd.DataFrame):
                nan_cols = X_transformed.columns[X_transformed.isnull().any()].tolist()
                error_msg = (
                    f"[FEATURE SELECTOR] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                    f"in selected features.\n"
                    f"NaN columns: {nan_cols[:20]}\n"
                    f"Method: {self.method}\n"
                    f"This indicates a problem with feature selection or input data."
                )
            else:
                error_msg = (
                    f"[FEATURE SELECTOR] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
