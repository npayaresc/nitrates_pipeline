...
class VAEReducer(DimensionReducer):
    ...
    def transform(self, X):
        ...
        logger.debug(
            "[VAE REDUCER] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after VAE dimensionality reduction
            error_msg = (
                f"[VAE REDUCER] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"in VAE-transformed features.\n"
                f"Latent dimensions: {self.latent_dim}\n"
                f"This indicates a problem with VAE transformation or input data."
            )
...
class DenoisingAutoencoderReducer(DimensionReducer):
    ...
    def transform(self, X):
        ...
        logger.debug(
            "[DENOISING AE REDUCER] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after Denoising AE dimensionality reduction
            error_msg = (
                f"[DENOISING AE REDUCER] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"in Denoising Autoencoder-transformed features.\n"
                f"Latent dimensions: {self.latent_dim}\n"
                f"This indicates a problem with Denoising Autoencoder transformation or input data."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)
...
class SparseAutoencoderReducer(DimensionReducer):
    ...
    def transform(self, X):
        ...
        logger.debug(
            "[SPARSE AE REDUCER] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            # CRITICAL ERROR: NaN values should not occur after Sparse AE dimensionality reduction
            error_msg = (
                f"[SPARSE AE REDUCER] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"in Sparse Autoencoder-transformed features.\n"
                f"Latent dimensions: {self.latent_dim}\n"
                f"This indicates a problem with Sparse Autoencoder transformation or input data."
            )
            logger.error(error_msg)
            raise ValueError(error_msg)
