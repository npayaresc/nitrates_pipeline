...
class SHAPBasedFeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if missing_features:
            logger.warning(
                f"\n⚠️  SHAP FEATURE MISMATCH DETECTED:\n"
            ...
            if len(valid_features) < 5:
                logger.error(
                    f"\n❌ CRITICAL: Only {len(valid_features)} SHAP features found in current data!\n"
        ...
        if self.n_features_selected_ < self.top_n:
            logger.warning(
                f"⚠️  Only {self.n_features_selected_} features selected "
    ...
    def transform(self, X):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            X_transformed = X[self.selected_feature_names_]
        else:
        ...
        logger.debug(
            "[SHAP FEATURE SELECTOR] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            logger.warning("[SHAP FEATURE SELECTOR] Found %d NaN values, filling with 0.0", nan_count)
            if isinstance(X_transformed, pd.DataFrame):
                X_transformed = X_transformed.fillna(0.0)
            else:
