"""
Enhanced Feature Engineering Module for Soil Nitrate Prediction
...
class MolecularBandDetector:
    ...
    def detect_cn_bands(self, wavelengths: np.ndarray, intensities: np.ndarray,
        ...
        wavelengths = np.atleast_1d(wavelengths).ravel()
        intensities = np.atleast_1d(intensities).ravel()

        # Verify shapes match
        if wavelengths.shape[0] != intensities.shape[0]:
            logger.warning(f"Wavelengths and intensities shape mismatch: {wavelengths.shape} vs {intensities.shape}. Returning NaN features.")
    ...
    def calculate_molecular_atomic_ratio(self, molecular_intensity: float,
        ...
        if np.isnan(molecular_intensity):
            logger.warning(f"[RATIO DEBUG] CN_P_molecular_ratio: molecular intensity (CN) is NaN")
            return np.nan
        if np.isnan(atomic_intensity):
            logger.warning(f"[RATIO DEBUG] CN_P_molecular_ratio: atomic intensity (P) is NaN")
            return np.nan
        if atomic_intensity < 1e-6:
            logger.warning(f"[RATIO DEBUG] CN_P_molecular_ratio: atomic intensity too small ({atomic_intensity:.2e})")
            return np.nan
        return molecular_intensity / atomic_intensity


class AdvancedRatioCalculator:
    """Calculates advanced elemental ratios for nitrate prediction."""
    ...
    def calculate_nutrient_ratios(self, features: Dict[str, float]) -> Dict[str, float]:
        """Calculate various nutrient ratios relevant to nitrate prediction and soil nutrient balance."""
        ratios = {}

        # Define key features for ratio calculations (updated for nitrate pipeline)
        key_features = ['K_I_simple_peak_height', 'K_I_peak_0', 'C_I_simple_peak_height', 'CA_II_393_simple_peak_height', 'N_I_primary_simple_peak_height']

        # Helper function for safe division with diagnostic logging
        def safe_ratio(num: float, den: float, name: str) -> float:
            if np.isnan(num):
                logger.warning(f"[RATIO DEBUG] {name}: numerator is NaN")
                return np.nan
            if np.isnan(den):
                logger.warning(f"[RATIO DEBUG] {name}: denominator is NaN")
                return np.nan
            if den < 1e-12:
                logger.warning(f"[RATIO DEBUG] {name}: denominator too small ({den:.2e})")
                return np.nan
        ...
        def get_intensity(features: Dict[str, float], element: str, metric: str = 'peak_height') -> float:
            ...
            if full_key in features:
                value = features[full_key]
                if np.isnan(value):
                    logger.warning(f"[FEATURE DEBUG] Element {element}: {full_key} exists but is NaN")
                else:
                    return value

            # Try simple feature format
            simple_key = f'{element}_simple_{metric}'
            if simple_key in features:
                value = features[simple_key]
                if np.isnan(value):
                    logger.warning(f"[FEATURE DEBUG] Element {element}: {simple_key} exists but is NaN")
                else:
                    return value

            # Log what we couldn't find (only once per element per session)
            if element not in self._missing_elements_logged:
                logger.warning(f"[FEATURE DEBUG] Element {element} not found in features (tried {full_key} and {simple_key})")
                logger.warning(f"[FEATURE DEBUG] Available features: {sorted([k for k in features.keys() if element.split('_')[0] in k])}")
                self._missing_elements_logged.add(element)
            return np.nan

        # Get peak intensities for nutrient context using available feature format
        ...
        # K_I_404 is not in nitrate config - skip these features
        k_404_intensity = np.nan  # Not available in nitrate pipeline
        k_404_2_intensity = np.nan  # Not available in nitrate pipeline

        # Context elements for nutrient balance (using nitrate pipeline element names)
        s_intensity = get_intensity(features, 'S_I')
        ca_intensity = get_intensity(features, 'CA_II_393')  # Nitrate config uses CA_II_393, not CA_I_help
        ca_ii_intensity = get_intensity(features, 'CA_II_393')  # Strong Ca II line at 393.37 nm
        p_intensity = get_intensity(features, 'P_I')  # Nitrate config uses P_I, not P_I_secondary

        # Extra debug for P_I since it's causing CN_P_molecular_ratio NaN
        if np.isnan(p_intensity):
            logger.warning(f"[P_I DEBUG] P_I returned NaN. Checking feature keys...")
            p_keys = [k for k in features.keys() if 'P_I' in k or 'P_' in k]
            logger.warning(f"[P_I DEBUG] P-related features available: {p_keys[:10]}")
            if 'P_I_peak_0' in features:
                logger.warning(f"[P_I DEBUG] P_I_peak_0 value: {features['P_I_peak_0']}")
            if 'P_I_simple_peak_height' in features:
                logger.warning(f"[P_I DEBUG] P_I_simple_peak_height value: {features['P_I_simple_peak_height']}")

        n_intensity = get_intensity(features, 'N_I_primary')  # Nitrate config uses N_I_primary, not N_I_help
        c_intensity = get_intensity(features, 'C_I')

        # Optional magnesium lines (for context, not primary target)
        # Note: Mg features are not in nitrate pipeline config - this is expected
        mg_285_intensity = np.nan  # Not in nitrate config
        mg_383_intensity = np.nan  # Not in nitrate config
        mg_ii_intensity = np.nan  # Not in nitrate config
        
        # Critical ratios for nitrate prediction - nutrient balance approach
        ...
        # K_766_404_ratio skipped - K_I_404 not in nitrate pipeline config
        ...
        # Combined K intensity (total potassium context signal)
        # Note: k_404 values excluded as K_I_404 not in nitrate pipeline
        k_total = sum(x for x in [k_primary_intensity, k_secondary_intensity] if not np.isnan(x))
        if k_total > 0:
...
class InterferenceCorrector:
    ...
    def detect_fe_interference(self, wavelengths: np.ndarray, intensities: np.ndarray,
        ...
        """Detect potential Fe interference on potassium context lines."""
        if fe_lines is None:
...
class PlasmaStateIndicators:
    ...
    def estimate_plasma_temperature_indicator(self, features: Dict[str, float]) -> float:
        ...
        Note: For nitrate pipeline, N_II and Mg features may not be available.
        Returns NaN if no valid ratios can be calculated.
        """
        # Get ionic and neutral line intensities (updated for nitrate pipeline)
        # N_II may not be in config - check if available
        n_ii = features.get('N_II_peak_0', np.nan)  # N II (ionic) - may not exist
        n_i = features.get('N_I_primary_peak_0', np.nan)  # N I (neutral) - using N_I_primary
        p_i = features.get('P_I_peak_0', np.nan)  # P I (neutral) - corrected name

        # Mg features (may not be in nitrate config)
        mg_ii = features.get('Mg_II_peak_0', np.nan)  # Mg II (ionic) - may not exist
        mg_i = features.get('Mg_I_285_peak_0', np.nan)  # Mg I (neutral) - may not exist

        # Calculate ratios (only if both values are valid)
        ratios = []
        if not (np.isnan(n_ii) or np.isnan(n_i)) and n_i > 0:
            ratios.append(n_ii / n_i)
...
class EnhancedSpectralFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X_features: Dict[str, float], wavelengths: np.ndarray, 
        ...
        if self.config.enable_molecular_bands:
            ...
            p_intensity = X_features.get('P_I_peak_0', np.nan)  # Nitrate config uses P_I, not P_I_secondary
        ...
        if self.config.enable_spectral_patterns:
            ...
            # Priority elements for nitrate prediction
        ...
        if self.config.enable_plasma_indicators:
            # Ionic/neutral ratios (updated for nitrate pipeline)
            n_ii = X_features.get('N_II_peak_0', np.nan)  # N_II may not be in nitrate config
            n_i = X_features.get('N_I_primary_peak_0', np.nan)  # Using N_I_primary instead of N_I_help
            enhanced_features['N_ionic_neutral_ratio'] = self.plasma_indicators.calculate_ionic_neutral_ratio(n_ii, n_i)

            # Temperature indicator
            enhanced_features['plasma_temp_indicator'] = self.plasma_indicators.estimate_plasma_temperature_indicator(X_features)

        # NaN statistics logging for enhanced features
        feature_values = list(enhanced_features.values())
        total_values = len(feature_values)
        nan_count = sum(1 for v in feature_values if isinstance(v, float) and np.isnan(v))
        nan_percentage = (nan_count / total_values * 100) if total_values > 0 else 0.0

        logger.debug(
            "[ENHANCED FEATURES] NaN Statistics: %d total NaNs (%.2f%% of %d total values)",
            nan_count, nan_percentage, total_values
        )

        if nan_count > 0:
            nan_features = [k for k, v in enhanced_features.items() if isinstance(v, float) and np.isnan(v)]

            # CRITICAL ERROR: NaN values should not occur in enhanced features
            error_msg = (
                f"[ENHANCED FEATURES] CRITICAL: Found {nan_count} NaN values ({nan_percentage:.2f}%) "
                f"across {len(nan_features)} features.\n"
                f"NaN features: {nan_features}\n"
                f"This indicates a problem with molecular bands, advanced ratios, spectral patterns, "
                f"interference correction, or plasma indicators calculation."
            )
