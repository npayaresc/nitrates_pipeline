...
Top 10 selected features by SHAP importance:
   1. N_I_primary_simple_peak_area                    SHAP=0.123456 (15.20%)
   2. N_I_primary_simple_peak_height                  SHAP=0.098765 (12.10%)
...
Check the `📊 Selection Statistics` in the output:
```
Selected features cover 78.5% of total SHAP importance
```
...
- **Performance**: Often similar or better R²
...
- **Performance**: Usually maintains R², sometimes improves
...
- `full_context` model → use `full_context` SHAP file
- `simple_only` model → use `simple_only` SHAP file

### Warning: "Only X features selected (requested N)"

```
⚠️  Only 25 features selected (requested 30).
...
- ✅ Train models with only the most important features
- ✅ Improve performance and interpretability
- ✅ Reduce training time significantly
- ✅ Create simpler, more robust models
- ✅ Iterate and refine feature selection
