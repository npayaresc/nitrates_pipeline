"""
Main Orchestration Script for the Nitrates Prediction Pipeline.
...
def run_model_optimization_pipeline(model_names: list, use_gpu: bool = False, use_raw_spectral: bool = False,
    ...
    if strategy in ["simple_only", "full_context", "N_only"]:
...
def run_range_specialist_pipeline(use_gpu: bool = False, validation_dir: Optional[str] = None,
                                 ...
                                 exclude_suspects_file: Optional[str] = None):
    """Executes the Range Specialist Neural Network optimization for low concentration nitrate range."""
    cfg = setup_pipeline_config(use_gpu, validation_dir, config_path)
    ...
    logger.info(f"Target: R² > 0.5 in low concentration nitrate range")
    if use_pca:
...
def run_single_prediction_pipeline(input_file: str, model_path: str):
    ...
    print(f"  Predicted Nitrate (NO3) Concentration: {prediction:.4f} %")
    print("--------------------------------\n")
...
def run_classification_pipeline(threshold=0.3, strategy="simple_only", models=None,
                               ...
                               exclude_suspects_file: Optional[str] = None):
    """
    Run the classification pipeline for nitrate level prediction.

    Args:
        threshold: Classification threshold for nitrate levels (default: 0.3)
...
def main():
    """Main entry point for the Nitrates Prediction ML Pipeline."""
    parser = argparse.ArgumentParser(description="Nitrates Prediction ML Pipeline")
    ...
    parser_train.add_argument("--strategy", type=str, 
                             choices=["full_context", "simple_only", "N_only"],
    ...
    parser_classify = subparsers.add_parser("classify", parents=[parent_parser], help="Train classification models for nitrate threshold prediction")
    parser_classify.add_argument("--threshold", type=float, default=0.3, help="Classification threshold for nitrate levels (default: 0.3)")
    ...
    parser_range_specialist = subparsers.add_parser("optimize-range-specialist", parents=[parent_parser], help="Optimize neural network for low concentration nitrate range (target R² > 0.5)")
